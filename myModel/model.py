...
class DVSA(torch.nn.Module):
    ...
    def forward(self, vis_feats, word_feats, entities_length, entities = None, ground_model = None, glove = None, boxes = None, DetectBox_class = None, DetectBox_score = None, DetectBox = None, args = None):
...
def train(train_loader, ground_model, glove, criterion, optimizer, epoch, args):
    ...
    #for batch_ind, (im_blobs, entities, entities_length, frm_length, rl_seg_inds, seg_nums, im_paths, img_ids) in enumerate(train_loader):
        ...
        D, D_sim, margin_loss = ground_model.DVSA( vis_feats, word_feats, entities_length,vid_entities,ground_model, glove, boxes, DetectBox_class, DetectBox_score, DetectBox, args)
...
def validate(val_loader, ground_model, glove, criterion, epoch, args):
    ...
    for batch_ind, (im_blobs, entities, entities_length, frm_length, rl_seg_inds, seg_nums, im_paths, img_ids) in enumerate(val_loader):
        ...
        if int((batch_ind + 1)%args.val_vis_freq) == 0:
            ...
            D = D.reshape(-1, Ne)
            out_imgs = []
            out_imgs = visualize_single_grounding(D, D_sim, vis_boxes, vid_ims, vid_entities, vid_im_paths)
